module house_renting::house_renting {
    ...
    // === Errors ===
    const E_TENANCY_INCORRECT: u64 = 1;
    const E_INVALID_SUI_AMOUNT: u64 = 2;
    const E_DAMAGE_INCORRECT: u64 = 3;
    const E_NO_PERMISSION: u64 = 4;
    const E_WRONG_PARAMS: u64 = 5;
    const E_INSPECTION_REVIEWED: u64 = 6;
    const E_INVALID_NOTICE: u64 = 7;
    ...
    // Represents the Rental platform administrator
    ...
    // Represents a house rental contract object
    ...
    // Represents an inspection report object. The landlord submits the inspection report, and the administrator reviews the inspection report
    ...
    // The landlord releases a rental message, creates a house object, and transfers it.
    ...
    public entry fun landlord_inspect(
        ...
        assert!(lease.landlord == tx_context::sender(ctx), E_NO_PERMISSION);
        assert!(damage >= DAMAGE_LEVEL_0 && damage <= DAMAGE_LEVEL_3, E_DAMAGE_INCORRECT);
    ...
    // The platform administrator reviews the inspection report and returns a coin of deposit
    public entry fun review_inspection_report(
        ...
        assert!(lease.house_id == inspection.house_id, E_WRONG_PARAMS);
        assert!(inspection.review_status == WAITING_FOR_REVIEW, E_INSPECTION_REVIEWED);
        assert!(damage >= DAMAGE_LEVEL_0 && damage <= DAMAGE_LEVEL_3, E_DAMAGE_INCORRECT);

        let deduct_deposit = calculate_deduct_deposit(lease.paid_deposit, damage);
    ...
    // Create a new rental platform object and initializes its fields.
    ...
    // The landlord releases a rental message, creates a rental notice object, and creates a house object
    public fun post_rental_notice(
        platform: &mut RentalPlatform,
        monthly_rent: u64,
        housing_area: u64,
        description: String,
        ...
        let rental_notice = RentalNotice {
            id: object::new(ctx),
            deposit,
            monthly_rent,
        ...
        table::add<ID, RentalNotice>(
            &mut platform.notices,
    ...
    public fun pay_rent(
        ...
        assert!(tenancy > 0, E_TENANCY_INCORRECT);
        assert!(
            table::contains<ID, RentalNotice>(&platform.notices, house_id),
            E_INVALID_NOTICE
        );
        ...
        assert!(total_fee == coin::value(&paid), E_INVALID_SUI_AMOUNT);

        // The deposit is stored by the rental platform
    ...
    fun tenant_return_house(
        ...
        assert!(
            lease.house_id == object::uid_to_inner(&house.id),
            E_WRONG_PARAMS
        );
        assert!(lease.tenant == tx_context::sender(ctx), E_NO_PERMISSION);
    ...
    fun calculate_deduct_deposit(paid_deposit: u64, damage: u8): u64 {
        let deduct_deposit = match damage {
            DAMAGE_LEVEL_1 => paid_deposit / 10,
            DAMAGE_LEVEL_2 => paid_deposit / 2,
            DAMAGE_LEVEL_3 => paid_deposit,
            _ => 0,
        };
    ...
    fun test_rent_house() {
        ...
        let house_description = String::from("This house faces north and south, with sufficient sunlight and good ventilation. It is also close to the subway station and has a favorable price.");
        let house_photo = String::from("https%3A%2F%2Fts1.cn.mm.bing.net%2Fth%3Fid%3DOIP-C.FNoLwTxiT7CM5e0mmMxD6AHaHT%26w%3D119%26h%3D150%26c%3D8%26rs%3D1%26qlt%3D90%26o%3D6%26pid%3D3.1%26rm%3D2");
        let damage: u8 = DAMAGE_LEVEL_1;
        let damage_description = String::from("The house is slightly damaged");
        let damage_photo = String::from("https%3A%2F%2Fts1.cn.mm.bing.net%2Fts1.cn.mm.bing.net%2Fth%3Fid%3DOIP-C.FNoLwTxiT7CM5e0mmMxD6AHaHT%26w%3D119%26h%3D150%26c%3D8%26rs%3D1%26qlt%3D90%26o%3D6%26pid%3D3.1%26rm%3D2");

        let scenario_val = test_scenario::begin(admin);
        let scenario = &mut scenario_val;

        // Admin creates a RentalPlatform share object and gets the Admin object
        ...
        test_scenario::next_tx(scenario, landlord);
        {
            ...
            post_rental_notice_and_transfer(
                platform_ref,
                house_monthly_rent,
                house_area,
                house_description.clone(),
                house_photo.clone(),
        ...
        test_scenario::next_tx(scenario, tenant);
        {
            ...
            let notice: &RentalNotice =
                table::borrow<ID, RentalNotice>(&platform_ref.notices, house_id);
            assert_eq(
                object::id_to_address(&notice.house_id),
                object::id_to_address(&house_id)
            );
        ...
        test_scenario::next_tx(scenario, landlord);
        {
            ...
            assert_eq(
                object::id_to_address(&lease.house_id),
                object::uid_to_address(&house.id)
            );
            assert_eq(lease.landlord, house.owner);
            assert_eq(
                table::contains<ID, RentalNotice>(&platform_ref.notices, object::uid_to_inner(&house.id)),
                false
            );
        ...
        test_scenario::next_tx(scenario, tenant);
        {
            ...
            let expect_deduct_deposit =
                calculate_deduct_deposit(lease.paid_deposit, inspection.damage);
            assert_eq(expect_deduct_deposit, inspection.deduct_deposit);

            tenant_return_house_and_transfer(
                platform_ref,
                &lease,
                house,
                test_scenario::ctx(scenario),
